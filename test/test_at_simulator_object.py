...
def _check_initial_phys_data(atsim, initial_phys_data):
    try:
        ...
        numpy.testing.assert_almost_equal(
            initial_phys_data["tune"][1], atsim.get_tune("y"), decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["chromaticity"][0], atsim.get_chromaticity("x"), decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["chromaticity"][1], atsim.get_chromaticity("y"), decimal=8
        )
        numpy.testing.assert_almost_equal(
        ...
        numpy.testing.assert_almost_equal(
            initial_phys_data["alpha"], atsim.get_alpha()[-1], decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["beta"], atsim.get_beta()[-1], decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["m44"], atsim.get_m44()[-1], decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["mu"], atsim.get_mu()[-1], decimal=8
        )
        numpy.testing.assert_almost_equal(
            initial_phys_data["rad_int"], atsim.get_radiation_integrals(), decimal=14
        )
        return True
    except Exception:
        return False
...
def test_recalculate_phys_data(atsim, initial_phys_data):
    ...
    numpy.testing.assert_almost_equal(tune, [0.37444833, 0.86048592], decimal=8)
    numpy.testing.assert_almost_equal(
        emit, [1.34308653e-10, 3.74339964e-13], decimal=15
    )
